...
#The quantitative form in Sørensen family is the percentage difference. The quantitative form in the Jaccard family is the Ruzicka index.
...
# rich : #Richness/abundance difference or nestedness matrix, class = 'dist'. With options "BJ", "BS" and "N", 'rich' contains nestedness indices. With option "N", the 'repl' and 'rich' values do not add up to 'D'. 
...
#5. Total richness/abundance diff. div. (or nestedness)/Total beta div. Name of the dissimilarity coefficient.
...

{
  ...
  if(!quant) {      # Binary data provided, or make the data binary
    ...
    # Sørensen-based (% difference) components
      for(i in 2:n) {
        for(j in 1:(i-1)) {
          repl[i,j] <- repl[i,j]/(K[i]+K[j])
          rich[i,j] <- rich[i,j]/(K[i]+K[j])
          # cat(K[i], K[j], W[i,j],"\n")
    ...
    if(coef==4) { # Decomposing Ruzicka in the spirit of Baselga 2013 
      for(i in 2:n) {
        for(j in 1:(i-1)) { 
          repl[i,j] <- 2*(min(K[i],K[j])-W[i,j])/(2*min(K[i],K[j])-W[i,j]) 
          rich[i,j] <- abs(K[i]-K[j])*W[i,j]/((K[i]+K[j]-W[i,j])*(2*min(K[i],K[j])-W[i,j])) 
          # cat(K[i], K[j], W[i,j],"\n")
