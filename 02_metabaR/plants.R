# Data processing of plants_embl_98
...
dt <- tabfiles_to_metabarlist(file_pcrs = "./03_metabaR/plants_pcrs.txt",
                              file_samples = "./03_metabaR/surfsedi_samples_corr.txt",
                              file_reads = "./03_metabaR/plants_r2_embl_98_reads_modified.txt",
                              file_motus = "./03_metabaR/plants_r2_embl_98_motus_modified.txt")
...
# flag pcrs with total contaminant relative abundance > 30% of reads)
dt$pcrs$low_contamination_level <- 
  ifelse(a$conta.relab[match(rownames(dt$pcrs), rownames(a))]>3e-1,  F, T)


# Flag MOTUs corresponding to target (TRUE) vs. non-target (FALSE) taxa 
dt$motus$target_taxon <- dt$motus$kingdom_name=="Viridiplantae" & !is.na(dt$motus$kingdom_name)


# Add a column "not_degraded"
dt$motus$not_degraded <-
  ifelse(dt$motus$BEST_IDENTITY < 0.98, F, T)


# Flag pcrs with an acceptable (T) or unacceptable sequencing depth (F)
dt$pcrs$seqdepth_ok <-ifelse(rowSums(dt$reads) < 1e4, F,T)
...
tmp <- subset_metabarlist(tmp, "motus", indices = rowSums(tmp$motus[,c("target_taxon", "not_extr_conta","not_degraded")])==3)
...
saveRDS(dt_clean, file = "./00_R-repository/plants_r2_embl_98_clean")
