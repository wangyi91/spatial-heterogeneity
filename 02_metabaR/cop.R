# Data processing of cop_r5_EmblSilvaCustom_85
...
dt <- tabfiles_to_metabarlist(file_pcrs = "./03_metabaR/cop_pcrs.txt",
                              file_samples = "./03_metabaR/surfsedi_samples_corr.txt",
                              file_reads = "./03_metabaR/cop_r5_EmblSilvaCustom_85_reads_modified.txt",
                              file_motus = "./03_metabaR/cop_r5_EmblSilvaCustom_85_motus_modified.txt")
...
dt$motus$target_taxon <- dt$motus$subclass_name=="Copepoda" & !is.na(dt$motus$subclass_name)


# Add a column "not_degraded"
dt$motus$not_degraded <-
  ifelse(dt$motus$BEST_IDENTITY < 0.85, F, T)
...
# remove BE124 because it is redundant of S25
dt_clean <- subset_metabarlist(dt_clean, "samples", rownames(dt_clean$samples) != "BE124")

# Save the cleaned and aggregated datasets for future reference
saveRDS(dt_clean, file = "./00_R-repository/cop_r5_EmblSilvaCustom_85_clean")
