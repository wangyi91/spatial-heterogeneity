...
dt <- tabfiles_to_metabarlist(file_pcrs = "./02_metabaR/cyano_pcrs.txt",
                              file_samples = "./02_metabaR/surfsedi_samples_corr.txt",
                              file_reads = "./02_metabaR/cyano_r2_silva_95_reads_modified.txt",
                              file_motus = "./02_metabaR/cyano_r2_silva_95_motus_modified.txt")
...
dt_clean <- tmp %>% subset_metabarlist(., "reads", rowSums(.$reads) > 0) %>%
  subset_metabarlist(.,"pcrs", 
                     indices = .$pcrs$type == "sample" & 
                       rowSums(.$pcrs[,c("low_contamination_level", "seqdepth_ok", "replicating_pcr")]) == 3 &
                                 is.na(.$pcrs[,c("low_contamination_level")])==F) 
