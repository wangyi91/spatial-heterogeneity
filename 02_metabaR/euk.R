# Data processing of euk_silva_97
...
dt <- tabfiles_to_metabarlist(file_pcrs = "./03_metabaR/euk_pcrs.txt",
                              file_samples = "./03_metabaR/surfsedi_samples_corr.txt",
                              file_reads = "./03_metabaR/euk_r2_silva_97_reads_modified.txt",
                              file_motus = "./03_metabaR/euk_r2_silva_97_motus_modified.txt")
...
# flag pcrs with total contaminant relative abundance > 15% of reads)
dt$pcrs$low_contamination_level <- 
  ifelse(a$conta.relab[match(rownames(dt$pcrs), rownames(a))]>1.5e-1,  F, T)


# Flag MOTUs corresponding to target (TRUE) vs. non-target (FALSE) taxa 
dt$motus$target_taxon <- dt$motus$superkingdom_name=="Eukaryota" & !is.na(dt$motus$superkingdom_name)
...
dt$pcrs$seqdepth_ok <-ifelse(rowSums(dt$reads) < 1e3, F,T)
...
saveRDS(dt_clean, file = "./00_R-repository/euk_r2_silva_97_clean")
